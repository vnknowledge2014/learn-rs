...
2. **Triệt tiêu kiểm tra biên giới hạn (Bounds Check Elimination)**: Trong vòng lặp `for i in 0..len` truyền thống, CPU phải so sánh `i < len` ở mỗi owner kỳ để tránh tràn ô nhớ. Với Iterator, Rust kiểm soát chặt chẽ điểm đầu và điểm cuối, cho phép trình tối ưu LLVM loại bỏ hoàn toàn các lệnh rẽ nhánh kiểm tra biên, đồng thời tự động vector hóa mã máy bằng các lệnh SIMD siêu tốc trên CPU hiện đại!
