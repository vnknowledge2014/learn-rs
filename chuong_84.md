...
pub fn tim_to_hop_dung(cac_chuoi: &[Vec<f64>], buoc: usize) -> Option<KetQuaDaBien> {
    let k = cac_chuoi.len();
    if k < 2 || buoc == 0 { return None; }
    let n = cac_chuoi.iter().map(|c| c.len()).min()?;
    if n < 30 { return None; }

    let mut tot_nhat: Option<KetQuaDaBien> = None;
    // Chuẩn hoá: trọng số đầu tiên luôn = 1, các trọng số sau quét trong [−2, 2]
    let mut ts = vec![1.0; k];
    let tong_to_hop = buoc.pow((k - 1) as u32);

    for ma in 0..tong_to_hop {
        let mut m = ma;
        for j in 1..k {
            let b = m % buoc;
            m /= buoc;
            ts[j] = -2.0 + 4.0 * b as f64 / (buoc - 1).max(1) as f64;
        }
        let chenh: Vec<f64> = (0..n)
            .map(|i| (0..k).map(|j| ts[j] * cac_chuoi[j][i]).sum())
            .collect();

        if let Some(kq) = kiem_dinh_dong_lien_ket(&chenh, -0.01) {
            let tot_hon = tot_nhat.as_ref()
                .map_or(true, |t| kq.he_so_keo_ve < t.he_so_keo_ve);
            if tot_hon && kq.he_so_keo_ve < 0.0 {
                tot_nhat = Some(KetQuaDaBien {
                    trong_so: ts.clone(),
                    he_so_keo_ve: kq.he_so_keo_ve,
                    nua_chu_ky: kq.nua_chu_ky,
                });
            }
        }
    }
...
pub fn dong_gop_rui_ro(trong_so: &[f64], hiep_phuong_sai: &[Vec<f64>]) -> Vec<f64> {
    let n = trong_so.len();
    // σ_p = √(wᵀ Σ w)
    let mut ps = 0.0;
    for i in 0..n { for j in 0..n {
        ps += trong_so[i] * trong_so[j] * hiep_phuong_sai[i][j];
    }}
    let sigma = ps.sqrt();
    if sigma <= 0.0 { return vec![0.0; n]; }

    (0..n).map(|i| {
        let bien: f64 = (0..n).map(|j| hiep_phuong_sai[i][j] * trong_so[j]).sum();
        trong_so[i] * bien / sigma        // đóng góp cận biên × trọng số
    }).collect()
}

/// Lặp cho tới khi mọi đóng góp rủi ro bằng nhau (có tương quan).
pub fn ngang_bang_rui_ro(hiep_phuong_sai: &[Vec<f64>], so_vong: usize) -> Vec<f64> {
    let n = hiep_phuong_sai.len();
    let mut w = vec![1.0 / n as f64; n];
    let muc_tieu = 1.0 / n as f64;

    for _ in 0..so_vong {
        let dg = dong_gop_rui_ro(&w, hiep_phuong_sai);
        let tong: f64 = dg.iter().sum();
        if tong <= 0.0 { break; }
        for i in 0..n {
            let ty_le = dg[i] / tong;
            // Đóng góp quá lớn → giảm trọng số, và ngược lại
            w[i] *= (muc_tieu / ty_le.max(1e-12)).powf(0.5);   // giảm chấn 0,5
        }
