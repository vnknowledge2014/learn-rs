...
Nhân id trận ngây thơ trên GPU bị chặn bởi **băng thông bộ nhớ**, không phải sức tính. Với mỗi phép nhân-cộng, bạn phải đọc 2 giá trị — tỉ lệ tính/đọc quá thấp.
...
impl MoPhongHistogram {
    ...
    pub fn theo_block(&self, data: &[u32]) -> KetQuaHistogram {
        let mut tong = vec![0u32; self.so_thung];
        let mut va_cham = 0u64;
        for khoi in data.chunks(self.amount_new_block) {
        ...
        KetQuaHistogram { count: tong, va_cham_nguyen_tu: va_cham }
    }
}
```
