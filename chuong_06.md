...
fn tieu_thu_chuoi(chuoi_nhan_vao: String) {
...
fn them_hau_to(mut chuoi: String) -> String {
    chuoi.push_str(" (Đã được kiểm định)");
    chuoi // Trả lại quyền sở hữu chuỗi mới về cho nơi gọi hàm
}

// 3. Hàm nhận kiểu Copy trên Stack: Không ảnh hưởng gì đến biến gốc
fn in_so_nguyen(so: i32) {
...
fn main() {
    ...
    let diem_so_goc = 100;
    let diem_so_sao_chep = diem_so_goc; // Tự động nhân bản trên Stack

    println!("- Điểm gốc: {}, Điểm sao chép: {}", diem_so_goc, diem_so_sao_chep);
    in_so_nguyen(diem_so_goc);
    // Biến diem_so_goc vẫn sử dụng hoàn toàn bình thường sau khi truyền vào hàm!
    println!("- Sau khi gọi hàm, điểm gốc vẫn còn nguyên: {}", diem_so_goc);

    // --- PHẦN 2: CƠ CHẾ DI CHUYỂN TRÊN HEAP (MOVE SEMANTICS) ---
    println!("\n2. Kiểm tra cơ chế Di chuyển quyền sở hữu (Move):");
    let chung_thu_so = String::from("CHUNG_THU_BAO_MAT_2026");
    println!("- Biến 'chung_thu_so' đang là chủ sở hữu hợp pháp duy nhất.");

    // Chuyển giao quyền sở hữu từ chung_thu_so sang chu_so_huu_moi:
    let chu_so_huu_moi = chung_thu_so;
    println!("- Đã sang tên đổi chủ thành công cho: {}", chu_so_huu_moi);

    // NẾU BẠN BỎ CHÚ THÍCH DÒNG LỆNH SAU, RUSTC SẼ BÁO LỖI E0382 NGAY:
    // println!("Thử dùng lại biến cũ: {}", chung_thu_so);

    // --- PHẦN 3: DI CHUYỂN VÀO HÀM VÀ MẤT QUYỀN SỞ HỮU ---
    println!("\n3. Chuyển quyền sở hữu vào một hàm con:");
    let thong_diep = String::from("Xin chào từ Hà Nội");
    
    // Khi gọi hàm này, thong_diep bị Move vào hàm con và biến mất khỏi main!
    tieu_thu_chuoi(thong_diep);

    // Dòng sau cũng bị lỗi E0382 vì thong_diep đã bị Drop bên trong hàm con:
    // println!("Thử in lại thông điệp: {}", thong_diep);

    // --- PHẦN 4: LẤY LẠI QUYỀN SỞ HỮU THÔNG QUA GIÁ TRỊ TRẢ VỀ ---
    println!("\n4. Chuyển giao đi và nhận lại quyền sở hữu qua return:");
    let ho_so = String::from("Hồ sơ ứng viên Nguyễn Văn A");
    let ho_so_da_duyet = them_hau_to(ho_so);
    // Lúc này 'ho_so' đã bị move, nhưng 'ho_so_da_duyet' là chủ nhân mới nắm giữ kết quả!
    println!("- Kết quả hồ sơ sau khi xử lý: {}", ho_so_da_duyet);

    // --- PHẦN 5: NHÂN BẢN SÂU BẰNG .clone() KHI CẦN THIẾT ---
    println!("\n5. Nhân bản sâu toàn diện bằng phương thức .clone():");
    let tai_lieu_goc = String::from("Bản quyền sở hữu trí tuệ");
    let tai_lieu_nhan_ban = tai_lieu_goc.clone(); // Cấp phát thêm một vùng nhớ Heap mới

    println!("- Bản gốc     : {}", tai_lieu_goc);
