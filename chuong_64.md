...
|---|---|---|
| `E0369: binary operation == cannot be applied` | `#[derive(PartialEq)]` trên `KetQuaLapLich` nhưng `TienTrinh` bên trong lại không có | Thêm `PartialEq` vào derive của **mọi** kiểu lồng bên trong |
...
| `E0382: use of moved value` | Truyền `Vec<TienTrinh>` vào `lap_lich_fcfs` rồi dùng lại | Mỗi thuật toán một bản sao: dùng closure `let tao = \|\| vec![...]` |
| `index out of bounds` (lúc chạy) | `chuoi[i + 1..]` khi `i` là phần tử cuối | Rust cho phép `chuoi[len..]` (lát cắt rỗng) — đây là lý do `thay_trang_toi_uu` không panic |
...
Chạy mô phỏng theo từng đơn vị thời gian. Ở **mỗi** đơn vị, chọn lại tiến trình có `con_lai` nhỏ nhất trong số đã tới. Ghi `bat_dau` lần đầu được chọn, `ket_thuc` khi `con_lai` về 0. Vì chọn lại mỗi bước, tiến trình đang chạy tự động bị tiếm quyền khi có ứng viên tốt hơn.
</details>
...
```rust
pub fn lap_lich_srtf(mut tt: Vec<TienTrinh>) -> KetQuaLapLich {
    let n = tt.len();
    let mut dong_ho = 0u64;
    let mut dtg = Vec::new();
    let mut xong = 0;
    ...
    while xong < n {
        ...
        let chon = (0..n)
            .filter(|&i| tt[i].con_lai > 0 && tt[i].thoi_diem_den <= dong_ho)
            .min_by_key(|&i| (tt[i].con_lai, tt[i].pid));
        match chon {
            Some(i) => {
                if tt[i].bat_dau.is_none() { tt[i].bat_dau = Some(dong_ho); }
                dtg.push((dong_ho, tt[i].pid));
    ...
    KetQuaLapLich {
        duong_thoi_gian: dtg,
        cho_trung_binh: tong_cho as f64 / n as f64,
        quay_vong_trung_binh: tong_qv as f64 / n as f64,
        tien_trinh: tt,
    }
}
```
...
```rust
pub fn thay_trang_clock(chuoi: &[u64], so_khung: usize) -> KetQuaThayTrang {
    let mut khung: Vec<(u64, bool)> = Vec::new(); // (số trang, bit tham chiếu)
    let mut kim = 0usize;
    let mut loi = 0;
    let mut lich_su = Vec::new();

    for &t in chuoi {
        match khung.iter().position(|&(p, _)| p == t) {
            Some(i) => khung[i].1 = true,            // trúng: cho "cơ hội thứ hai"
            None => {
                loi += 1;
                if khung.len() < so_khung {
                    khung.push((t, true));
                    ...
                    while khung[kim].1 {
                        khung[kim].1 = false;        // xóa bit, cho qua lần này
                        kim = (kim + 1) % so_khung;
                    }
                    khung[kim] = (t, true);
                    kim = (kim + 1) % so_khung;
        ...
        lich_su.push(khung.iter().map(|&(p, _)| p).collect());
    }
    KetQuaThayTrang { so_loi_trang: loi, chuoi_khung: lich_su }
}
```
...
pub fn trang_thai_an_toan(
    ...
    let mut xong = vec![false; n];
    let mut thu_tu = Vec::new();

    for _ in 0..n {
        // tìm một tiến trình có thể hoàn thành với tài nguyên đang rảnh
        let ung_vien = (0..n).find(|&i| {
            !xong[i] && (0..m).all(|j| toi_da[i][j] - da_cap[i][j] <= ranh[j])
        });
        match ung_vien {
            Some(i) => {
