...
**Transform** gồm lọc, thêm cột dẫn xuất, và **xử lý giá trị thiếu** (`GiaTri::Rong`). Quyết định điền thiếu bằng gì (0, trung bình, giá trị trước đó) là một quyết định *nghiệp vụ*, không phải kỹ thuật — và nó ảnh hưởng lớn tới kết quả phân tích.
...
- Gộp các giá trị của cùng một khóa bằng một **vị nhóm tích bốn thành phần** (Chương 18): `(Tong, Dem, NhoNhat, LonNhat)`.
...
`nhom_va_tong_hop` tính tổng/trung bình/min/max. Trung vị bền vững hơn với điểm lạ. Viết `trung_vi(so: &[f64]) -> f64` và một bài test.
...
mod bt1 {
    ...
    fn trung_vi_khong_bi_diem_la_keo_lech() {
        ...
        let du_lieu = [1.0, 2.0, 3.0, 4.0, 1000.0];
        assert_eq!(trung_vi(&du_lieu), 3.0);       // ổn định
        let tb = du_lieu.iter().sum::<f64>() / 5.0;
...
**Bài tập 2 (Left join)**
`inner_join` chỉ giữ hàng khớp cả hai bên. Viết `left_join` giữ **mọi** hàng bảng trái; hàng không khớp thì các cột phải điền `GiaTri::Rong`. Test với "Đà Nẵng" không có dân số.

<details>
<summary><b>Gợi ý</b></summary>

Giống `inner_join` nhưng khi không tìm thấy khóa ở chỉ mục phải, vẫn thêm hàng trái và đệm `GiaTri::Rong` cho đủ số cột phải. Đây là join hay dùng nhất khi làm giàu (enrich) dữ liệu — giữ nguyên bảng chính, gắn thêm thông tin nếu có.
</details>
