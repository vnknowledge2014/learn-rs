...
`fib_ngay_tho` (O(2ⁿ)) so với `fib_qhd` (O(n), O(1) bộ nhớ) trong mã dưới là minh họa rõ nhất: cùng công thức, khác cách tổ chức tính toán, chênh nhau *hàng tỷ lần* ở n=50.
...
pub fn fib_ngay_tho(n: u64) -> u64 {
    if n < 2 { n } else { fib_ngay_tho(n - 1) + fib_ngay_tho(n - 2) }
}
...
pub fn doi_tien(cac_menh_gia: &[u64], so_tien: u64) -> Option<u64> {
...
pub fn day_con_chung_dai_nhat(a: &str, b: &str) -> usize {
...
pub fn ba_lo(trong_luong: &[u64], gia_tri: &[u64], suc_chua: u64) -> u64 {
    let n = trong_luong.len();
    let w = suc_chua as usize;
    let mut dp = vec![0u64; w + 1];
    for i in 0..n {
        // duyệt NGƯỢC để mỗi món chỉ dùng 1 lần (0/1)
        for cap in (trong_luong[i] as usize..=w).rev() {
            dp[cap] = dp[cap].max(dp[cap - trong_luong[i] as usize] + gia_tri[i]);
        }
    }
    dp[w]
}
...
pub fn hoan_vi<T: Clone>(cac_phan_tu: &[T]) -> Vec<Vec<T>> {
    let mut ket_qua = Vec::new();
    let mut hien_tai = Vec::new();
    let mut da_dung = vec![false; cac_phan_tu.len()];
    quay_lui_hoan_vi(cac_phan_tu, &mut da_dung, &mut hien_tai, &mut ket_qua);
    ket_qua
}
fn quay_lui_hoan_vi<T: Clone>(
    pt: &[T], da_dung: &mut [bool], hien_tai: &mut Vec<T>, kq: &mut Vec<Vec<T>>,
) {
    if hien_tai.len() == pt.len() {
        kq.push(hien_tai.clone());
        return;
    }
    for i in 0..pt.len() {
        if da_dung[i] { continue; }
        da_dung[i] = true;
        hien_tai.push(pt[i].clone());
        quay_lui_hoan_vi(pt, da_dung, hien_tai, kq);
        hien_tai.pop();        // LÙI LẠI
...
pub fn n_hau(n: usize) -> usize {
    ...
    dat_hau(0, n, &mut cot, &mut cheo_xuoi, &mut cheo_nguoc)
}
fn dat_hau(hang: usize, n: usize, cot: &mut [bool], cx: &mut [bool], cn: &mut [bool]) -> usize {
    if hang == n { return 1; }
    let mut so_cach = 0;
    for c in 0..n {
        let d1 = hang + c;
        ...
        so_cach += dat_hau(hang + 1, n, cot, cx, cn);
...
pub fn chon_hoat_dong(mut khoang: Vec<(u32, u32)>) -> usize {
    khoang.sort_by_key(|&(_, ket_thuc)| ket_thuc);
    let mut dem = 0;
    let mut het_gio = 0;
    for (bat_dau, ket_thuc) in khoang {
        if bat_dau >= het_gio {
            dem += 1;
            het_gio = ket_thuc;
        }
...
pub fn doi_tien_tham_lam(mut menh_gia: Vec<u64>, mut so_tien: u64) -> u64 {
...
/// Tính (co_so^so_mu) % modulo trong O(log so_mu).
pub fn luy_thua_mod(mut co_so: u64, mut so_mu: u64, modulo: u64) -> u64 {
    if modulo == 1 { return 0; }
    let mut kq = 1u64;
    co_so %= modulo;
    while so_mu > 0 {
        if so_mu & 1 == 1 {
            kq = (kq as u128 * co_so as u128 % modulo as u128) as u64;
        }
        so_mu >>= 1;
        co_so = (co_so as u128 * co_so as u128 % modulo as u128) as u64;
    }
    kq
}
...
fn main() {
    ...
    println!("   Đổi tiền [1,5,6,9] cho 11: {:?} xu (tối ưu)", doi_tien(&[1, 5, 6, 9], 11));
    println!("   LCS(\"ABCBDAB\", \"BDCAB\"): {}", day_con_chung_dai_nhat("ABCBDAB", "BDCAB"));
    println!("   Ba lô (tl=[1,3,4,5], gt=[1,4,5,7], sức chứa 7): {}",
             ba_lo(&[1, 3, 4, 5], &[1, 4, 5, 7], 7));

    println!("\n2. QUAY LUI");
    println!("   Số hoán vị của [1,2,3]: {}", hoan_vi(&[1, 2, 3]).len());
    for n in [4, 5, 6, 8] {
    ...
    println!("   Xếp nhiều cuộc họp nhất: {} cuộc (=(1,3),(4,7),(8,10))", chon_hoat_dong(hop));
    println!("   ⚠ Đổi tiền THAM LAM [1,3,4] cho 6: {} xu (SAI!)", doi_tien_tham_lam(vec![1, 3, 4], 6));
    println!("     Đổi tiền QHĐ    [1,3,4] cho 6: {:?} xu (ĐÚNG)", doi_tien(&[1, 3, 4], 6));

    println!("\n4. LÝ THUYẾT SỐ");
    println!("   ƯCLN(48, 36) = {}, BCNN = {}", ucln(48, 36), bcnn(48, 36));
    println!("   Số nguyên tố < 30: {:?}", sang_nguyen_to(30));
    println!("   (7^256) mod 13 = {}", luy_thua_mod(7, 256, 13));
...
mod kiem_thu {
    ...
    fn fib_hai_cach_khop_nhau() {
        for n in 0..=20 {
            assert_eq!(fib_ngay_tho(n), fib_qhd(n), "lệch ở n={}", n);
        }
    ...
    #[test]
    fn doi_tien_qhd() {
        assert_eq!(doi_tien(&[1, 5, 6, 9], 11), Some(2)); // 5+6
        assert_eq!(doi_tien(&[2], 3), None);              // không thể
        assert_eq!(doi_tien(&[1, 3, 4], 6), Some(2));     // 3+3
        assert_eq!(doi_tien(&[1, 2, 5], 0), Some(0));     // 0 tiền = 0 xu
    }

    #[test]
    fn tham_lam_doi_tien_co_the_sai() {
        // Đây là bằng chứng: tham lam KHÔNG tối ưu với mệnh giá [1,3,4]
        assert_eq!(doi_tien_tham_lam(vec![1, 3, 4], 6), 3); // 4+1+1
        assert_eq!(doi_tien(&[1, 3, 4], 6), Some(2));        // 3+3 -> QHĐ đúng
        assert!(doi_tien_tham_lam(vec![1, 3, 4], 6) as u64 > doi_tien(&[1, 3, 4], 6).unwrap());
    }

    #[test]
    fn lcs_dung() {
        assert_eq!(day_con_chung_dai_nhat("ABCBDAB", "BDCAB"), 4); // "BCAB" hoặc "BDAB"
        assert_eq!(day_con_chung_dai_nhat("abc", "abc"), 3);
        assert_eq!(day_con_chung_dai_nhat("abc", "xyz"), 0);
        assert_eq!(day_con_chung_dai_nhat("", "abc"), 0);
    }
    ...
    #[test]
    fn hoan_vi_dung_so_luong() {
        assert_eq!(hoan_vi(&[1, 2, 3]).len(), 6);   // 3! = 6
        assert_eq!(hoan_vi(&[1, 2, 3, 4]).len(), 24); // 4! = 24
        assert_eq!(hoan_vi::<i32>(&[]).len(), 1);   // hoán vị của rỗng = 1 (dãy rỗng)
    }
    ...
    fn chon_hoat_dong_tham_lam_dung() {
        ...
        let hop = vec![(1, 3), (2, 5), (4, 7), (1, 8), (5, 9), (8, 10)];
        assert_eq!(chon_hoat_dong(hop), 3); // (1,3),(4,7),(8,10)
    }
    ...
    fn luy_thua_mod_dung() {
        assert_eq!(luy_thua_mod(2, 10, 1000), 24);   // 1024 % 1000
        assert_eq!(luy_thua_mod(3, 0, 7), 1);        // x^0 = 1
        assert_eq!(luy_thua_mod(7, 256, 13), 9);
        // không tràn số dù số mũ lớn
        assert_eq!(luy_thua_mod(123456789, 987654321, 1_000_000_007), 652541198);
    }
}
```
...
pub fn to_hop(n: u32, k: u32) -> Vec<Vec<u32>> {
    let mut kq = Vec::new();
    let mut hien_tai = Vec::new();
    fn di(bat_dau: u32, n: u32, k: u32, ht: &mut Vec<u32>, kq: &mut Vec<Vec<u32>>) {
