...
**Lỗi 4 — Nhà tạo lập vượt hạn mức tồn kho.** Bản `TaoLapDonGian` chỉ đếm vị thế **đã khớp**. Nhưng lệnh đang treo cũng là rủi ro. Kết quả: vị thế chạm −900 dù hạn mức là 300. Sửa bằng `TaoLapCoKiemSoat` — theo dõi cả phơi nhiễm đang chờ. Bản sai được **giữ lại** làm ví dụ phản chứng có kiểm thử.
...
Điều đó chỉ đúng nếu chiến lược **không bao giờ đọc đồng hồ thật**. Đây là lý do kiến trúc quan trọng: mọi thành phần phải nhận thời gian qua tham số, không tự gọi `Instant::now()`. Trong Rust, cách ép buộc điều này là truyền `&DongHoAo` vào và không cho phép truy cập nào khác.

### 4. Tồn kho: đếm cả những gì chưa xảy ra

`TaoLapDonGian` sai vì nó chỉ đếm vị thế đã khớp. Nhưng nếu bạn đang treo 5 lệnh bán, mỗi lệnh 100 đơn vị, thì rủi ro thật của bạn là vị thế hiện tại **cộng thêm** 500 đơn vị bán tiềm năng.
...
| `E0502: cannot borrow as mutable` | Duyệt `self.lenh_thi_truong` rồi muốn sửa | Thu thay đổi vào `Vec` cục bộ, áp dụng sau vòng lặp |
| `E0499: two mutable borrows` | `self.so_lenh` và `self.chien_luoc` cùng lúc | Tách thành hàm nhận hai `&mut` riêng, hoặc `split_at_mut` |
...
impl MoHinhTacDong {
    ...
    pub fn tac_dong_diem_co_ban(&self, khoi_luong: u64) -> f64 {
        if self.khoi_luong_ngay <= 0.0 { return 0.0; }
        let ty_le = khoi_luong as f64 / self.khoi_luong_ngay;
        self.he_so_tam_thoi * ty_le.sqrt() * 10_000.0
    }

    /// Giá khớp thực tế sau khi tính tác động.
    pub fn gia_khop(&self, gia_yet: Gia, chieu: Chieu, khoi_luong: u64) -> Gia {
        let bp = self.tac_dong_diem_co_ban(khoi_luong);
        let dich = gia_yet as f64 * bp / 10_000.0;
        match chieu {
            Chieu::Mua => (gia_yet as f64 + dich) as Gia,   // mua thì đẩy giá lên
            Chieu::Ban => (gia_yet as f64 - dich) as Gia,
        }
    }

    /// Phần tác động KHÔNG hồi lại — cái này mới thực sự đắt.
    pub fn dich_vinh_vien(&self, gia_yet: Gia, khoi_luong: u64) -> f64 {
        gia_yet as f64 * self.tac_dong_diem_co_ban(khoi_luong)
...
fn bam_ket_qua(cac_lenh: &[(u64, Chieu, Gia, u64)]) -> u64 {
...
fn phat_lai_phai_tat_dinh() {
    let phien = sinh_phien(20_000, 42);

    let chay = || {
        let mut bpl = BoPhatLai::moi(phien.clone(), TocDoPhat::VoHan,
                                     MoHinhDoTre::dien_hinh());
        let mut cl = TaoLapCoKiemSoat::moi(300);
        bpl.chay(&mut cl);
