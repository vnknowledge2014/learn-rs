...
```rust
pub struct DinhTuyen { pub cac_be: Vec<BeThanhKhoan> }
...
impl DinhTuyen {
    ...
    pub fn chia_lenh(&self, tong: SoLuong, so_lat: usize) -> Vec<SoLuong> {
        let mut phan = vec![0 as SoLuong; self.cac_be.len()];
        if so_lat == 0 || self.cac_be.is_empty() { return phan; }
        let mut tam = self.cac_be.clone();
        let lat = tong / so_lat as SoLuong;
        if lat == 0 { return phan; }

        for _ in 0..so_lat {
            let mut tot_nhat = None;
            let mut nhieu_nhat: SoLuong = 0;
            for (i, be) in tam.iter().enumerate() {
                if let Ok(ra) = be.thu_hoan_doi_x_lay_y(lat) {
                    if tot_nhat.is_none() || ra > nhieu_nhat { nhieu_nhat = ra; tot_nhat = Some(i); }
                }
            }
            let i = match tot_nhat { Some(i) => i, None => break };
            // Cập nhật trạng thái bể: lát sau sẽ thấy giá đã xấu đi
            let _ = tam[i].hoan_doi_x_lay_y(lat, 0);
            phan[i] += lat;
        }
        phan
    }

    pub fn tong_nhan_duoc(&self, phan: &[SoLuong]) -> SoLuong {
        let mut tam = self.cac_be.clone();
        phan.iter().enumerate()
            .filter(|(_, &v)| v > 0)
            .filter_map(|(i, &v)| tam[i].hoan_doi_x_lay_y(v, 0).ok())
            .sum()
    }
}
```
...
```rust
#[derive(Debug, Clone)]
pub struct LenhLo { pub mua_token_x: bool, pub khoi_luong: SoLuong, pub gia_gioi_han: f64 }

#[derive(Debug)]
pub struct KetQuaLo {
    pub gia_thanh_toan: f64,
    pub khop_truc_tiep: SoLuong,
    pub du_qua_be: SoLuong,
    pub so_lenh_duoc_khop: usize,
}

pub fn giai_lo(cac_lenh: &[LenhLo], be: &BeThanhKhoan) -> KetQuaLo {
    let gia_be = be.gia_x();

    // Lệnh nào chấp nhận được giá thanh toán chung thì tham gia lô.
    let mua: Vec<&LenhLo> = cac_lenh.iter()
        .filter(|l| l.mua_token_x && l.gia_gioi_han >= gia_be).collect();
    let ban: Vec<&LenhLo> = cac_lenh.iter()
        .filter(|l| !l.mua_token_x && l.gia_gioi_han <= gia_be).collect();

    let tong_mua: SoLuong = mua.iter().map(|l| l.khoi_luong).sum();
    let tong_ban: SoLuong = ban.iter().map(|l| l.khoi_luong).sum();
    ...
    KetQuaLo {
        ...
        khop_truc_tiep,
        du_qua_be: du,
        so_lenh_duoc_khop: mua.len() + ban.len(),
    }
}
```
