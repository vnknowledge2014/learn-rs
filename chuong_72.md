...
impl TokenCw20 {
    pub fn increase_allowance(store: &mut Store, owner: &str, can: &str, them: Money)
        -> Result<Response, ContractError>
    {
        ...
        Ok(Response::new().event("increase_allowance", &[("owner", owner), ("duoc", can)]))
    }

    /// Giảm về 0 nếu trừ quá — an toàn hơn báo lỗi, vì ý định của người dùng
    /// ("bớt quyền") vẫn được thực hiện trọn vẹn.
    pub fn decrease_allowance(store: &mut Store, owner: &str, can: &str, bot: Money) -> Response {
        let k = Self::key_allowance(owner, can);
        let new = TokenCw20::allowance(store, owner, can).saturating_sub(bot);
        store.set(&k, &new.to_be_bytes());
        Response::new().event("decrease_allowance", &[("owner", owner), ("duoc", can)])
    }

    pub fn transfer_from(store: &mut Store, caller: &str, owner: &str, den: &str, quantity: Money)
        -> Result<Response, ContractError>
    {
        let han = TokenCw20::allowance(store, owner, caller);
        if han < quantity { return Err(ContractError::Forbidden { ai: caller.into() }); }
        ...
        if co < quantity { return Err(ContractError::InsufficientFunds { can: quantity, co }); }
        ...
        Ok(Response::new().event("transfer_from", &[("tu", owner), ("den", den)]))
    }
}
```
...
pub fn check_all(
    ...
    if account.len() < constraints.len() {
        return Err(SolanaError::MissingAccount(constraints.len()));
    }
    for (account, rb) in account.iter().zip(constraints) {
        for yc in rb.requirements {
            match yc {
                Request::KyTen => CheckAccount::must_ky(account)?,
                Request::GhiDuoc => CheckAccount::must_record_can(account)?,
                Request::ThuocChuongTrinh =>
                    CheckAccount::must_owned_own(account, program_id)?,
                Request::DungDinhDanh(d) => {
                    ...
                    if account.data.first() != Some(d) {
                        return Err(SolanaError::WrongOwner {
                            account: account.address.clone(),
                            expected: format!("kieu#{}", d),
