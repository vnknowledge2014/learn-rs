...
impl TokenCw20 {
    pub fn tang_han_muc(store: &mut Store, chu: &str, can: &str, them: SoTien)
        -> Result<Response, ContractError>
    {
        let k = Self::key_allowance(chu, can);
        let cu = TokenCw20::allowance(store, chu, can);
        let new = cu.checked_add(them).ok_or(ContractError::TranSo)?;
        store.set(&k, &new.to_be_bytes());
        Ok(Response::new().event("tang_han_muc", &[("chu", chu), ("duoc", can)]))
    }

    /// Giảm về 0 nếu trừ quá — an toàn hơn báo lỗi, vì ý định của người dùng
    /// ("bớt quyền") vẫn được thực hiện trọn vẹn.
    pub fn giam_han_muc(store: &mut Store, chu: &str, can: &str, bot: SoTien) -> Response {
        let k = Self::key_allowance(chu, can);
        let new = TokenCw20::allowance(store, chu, can).saturating_sub(bot);
        store.set(&k, &new.to_be_bytes());
        Response::new().event("giam_han_muc", &[("chu", chu), ("duoc", can)])
    }

    pub fn chuyen_ho(store: &mut Store, nguoi_call: &str, chu: &str, den: &str, quantity: SoTien)
        -> Result<Response, ContractError>
    {
        let han = TokenCw20::allowance(store, chu, nguoi_call);
        if han < quantity { return Err(ContractError::KhongCoQuyen { ai: nguoi_call.into() }); }

        // Trừ hạn mức TRƯỚC khi chuyển tiền — mẫu "kiểm tra – tác động – tương tác".
        // Thứ tự này đã ngăn được vô số vụ tấn công tái nhập.
        store.set(&Self::key_allowance(chu, nguoi_call), &(han - quantity).to_be_bytes());

        let co = store.balance(chu);
        if co < quantity { return Err(ContractError::KhongDuSoDu { can: quantity, co }); }
        store.set_balance(chu, co - quantity);
        let nhan = store.balance(den);
        store.set_balance(den, nhan.checked_add(quantity).ok_or(ContractError::TranSo)?);

        Ok(Response::new().event("chuyen_ho", &[("tu", chu), ("den", den)]))
    }
}
```
...
pub fn kiem_tra_tat_ca(
    ...
    for (tk, rb) in account.iter().zip(rang_buoc) {
        for yc in rb.cac_yeu_cau {
            match yc {
                Request::KyTen => CheckAccount::must_ky(tk)?,
                Request::GhiDuoc => CheckAccount::must_record_can(tk)?,
                Request::ThuocChuongTrinh =>
                    CheckAccount::must_owned_own(tk, id_chuong_trinh)?,
                Request::DungDinhDanh(d) => {
                    ...
                    if tk.data.first() != Some(d) {
                        return Err(LoiSolana::KhongPhaiChuSoHuu {
                            account: tk.address.clone(),
                            mong_doi: format!("kieu#{}", d),
                            thuc_te: format!("kieu#{:?}", tk.data.first()),
                        });
                    }
                }
            }
