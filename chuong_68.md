...
```rust
tich_luy += delta_thuc;
while tich_luy >= BUOC_CO_DINH {
    tich_luy -= BUOC_CO_DINH;
    cap_nhat_vat_ly(BUOC_CO_DINH);
}
he_so_noi_suy = tich_luy / BUOC_CO_DINH;   // phần dư → làm mượt hình ảnh
```
...
#[derive(Resource)]
struct DongHo(BoTichLuyNguyen);

fn he_thong_vat_ly_bevy(
    thoi_gian: Res<Time>,
    mut dong_ho: ResMut<DongHo>,
    mut truy_van: Query<(&mut Transform, &mut VanToc)>,
) {
    let nhip = dong_ho.0.khung_moi(thoi_gian.delta().as_nanos() as u64);
    for _ in 0..nhip.so_buoc_vat_ly {
        for (mut vi_tri, mut van_toc) in truy_van.iter_mut() {
            // Gọi thẳng hàm THUẦN TÚY đã kiểm thử ở trên
            let the = TheVatLy {
                vi_tri: Vec2::moi(vi_tri.translation.x, vi_tri.translation.y),
                van_toc: van_toc.0,
                khoi_luong: 1.0,
            };
            let moi = buoc_euler_nua_an(the, Vec2::moi(0.0, -9.81), BUOC_CO_DINH);
            vi_tri.translation.x = moi.vi_tri.x;
            vi_tri.translation.y = moi.vi_tri.y;
            van_toc.0 = moi.van_toc;
        }
    }
}
...
fn main() {
    App::new()
        ...
        .insert_resource(DongHo(BoTichLuyNguyen::moi(60)))
        .add_systems(Update, he_thong_vat_ly_bevy)
        .run();
}
```

Đây chính là giá trị của "lõi hàm, vỏ mệnh lệnh": bạn kiểm thử được `buoc_euler_nua_an` và `va_cham_qua_luoi` bằng `cargo test` chạy trong mili-giây, không cần mở cửa sổ, không cần GPU, không cần người ngồi bấm thử.
...
|---|---|---|
| `E0502: cannot borrow tg as mutable ... also as immutable` | `for e in tg.co_vi_tri_va_van_toc()` rồi `tg.vi_tri.get_mut(&e)` | Thu thập danh sách thực thể ra `Vec` **trước**, rồi mới lặp |
| `E0499: cannot borrow tg as mutable more than once` | Lấy `&mut tg.vi_tri` và `&mut tg.van_toc` cùng lúc | Đọc vận tốc ra biến cục bộ trước, rồi mới mượn vị trí |
...
| NaN lan khắp mô phỏng | `chuan_hoa()` một vector không (chia cho 0) | Kiểm tra `if d < 1e-6 { return Vec2::KHONG }` |
...
impl CayTuPhan {
    pub fn moi(vung: HopBao, suc_chua: usize, do_sau_toi_da: u32) -> Self {
        CayTuPhan { vung, vat: Vec::new(), con: None, suc_chua, do_sau_toi_da }
    }

    pub fn chen(&mut self, chi_so: usize, hop: &HopBao) {
        if !self.vung.giao_nhau(hop) { return; }
        ...
        if self.vat.len() > self.suc_chua && self.do_sau_toi_da > 0 {
            self.chia(hop);
        }
    }

    fn chia(&mut self, _h: &HopBao) {
        let t = self.vung.tam();
        let (min, max) = (self.vung.min, self.vung.max);
        let goc = |a: Vec2, b: Vec2| HopBao { min: a, max: b };
        let mut con = [
            CayTuPhan::moi(goc(min, t), self.suc_chua, self.do_sau_toi_da - 1),
            CayTuPhan::moi(goc(Vec2::moi(t.x, min.y), Vec2::moi(max.x, t.y)),
                           self.suc_chua, self.do_sau_toi_da - 1),
            CayTuPhan::moi(goc(Vec2::moi(min.x, t.y), Vec2::moi(t.x, max.y)),
                           self.suc_chua, self.do_sau_toi_da - 1),
            CayTuPhan::moi(goc(t, max), self.suc_chua, self.do_sau_toi_da - 1),
        ];
        // (trong bản đầy đủ, cần giữ &[HopBao] để phân phối lại chính xác)
...
```rust
pub fn va_cham_dan_hoi(a: &mut TheVatLy, b: &mut TheVatLy, bk_a: f32, bk_b: f32) -> bool {
    let delta = b.vi_tri.tru(a.vi_tri);
    let kc_bp = delta.do_dai_binh_phuong();
    ...
    let tong_m = a.khoi_luong + b.khoi_luong;
    a.vi_tri = a.vi_tri.tru(phap_tuyen.nhan(chong * b.khoi_luong / tong_m));
    b.vi_tri = b.vi_tri.cong(phap_tuyen.nhan(chong * a.khoi_luong / tong_m));

    // BƯỚC 2: chỉ đổi thành phần vận tốc DỌC pháp tuyến
    let u1 = a.van_toc.tich_vo_huong(phap_tuyen);
    let u2 = b.van_toc.tich_vo_huong(phap_tuyen);
    if u1 - u2 <= 0.0 { return true; } // đang tách xa nhau rồi, đừng "hút" lại

    let (m1, m2) = (a.khoi_luong, b.khoi_luong);
    let v1 = ((m1 - m2) * u1 + 2.0 * m2 * u2) / tong_m;
    let v2 = ((m2 - m1) * u2 + 2.0 * m1 * u1) / tong_m;

    a.van_toc = a.van_toc.cong(phap_tuyen.nhan(v1 - u1));
    b.van_toc = b.van_toc.cong(phap_tuyen.nhan(v2 - u2));
    true
}
```
...
Đổi `ThucThe` từ `u32` thành `struct ThucThe { chi_so: u32, the_he: u32 }`. Khi hủy một thực thể, tăng thế hệ của ô đó lên. Khi cấp lại ô đó cho thực thể mới, nó mang thế hệ mới.
...
pub struct TheGioiV2 {
    ...
    pub vi_tri: HashMap<ThucTheV2, Vec2>,
}

impl TheGioiV2 {
    pub fn moi() -> Self {
        TheGioiV2 { the_he: Vec::new(), con_song: Vec::new(),
                    o_trong: Vec::new(), vi_tri: HashMap::new() }
    }
    ...
    pub fn huy(&mut self, e: ThucTheV2) {
