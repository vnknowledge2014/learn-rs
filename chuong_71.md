...
| `leading_zeros` cho 256 | XOR của một nút với chính nó bằng 0 | Xử lý riêng trường hợp `khoang_cach == 0` |
...
pub struct MangChongEntropy {
    ...
    pub da_thay: BTreeMap<MaNut, std::collections::BTreeSet<u64>>,
}

impl MangChongEntropy {
    pub fn moi(cac_nut: &[MaNut]) -> Self {
        MangChongEntropy {
            da_thay: cac_nut.iter().map(|&n| (n, Default::default())).collect(),
        }
    }

    pub fn gieo(&mut self, nut: MaNut, thong_diep: u64) {
        self.da_thay.entry(nut).or_default().insert(thong_diep);
    }

    /// Đồng bộ hai chiều giữa hai nút. Trả về số thông điệp đã trao đổi.
    pub fn chong_entropy(&mut self, a: MaNut, b: MaNut) -> usize {
        if a == b { return 0; }
        let (ca, cb) = match (self.da_thay.get(&a), self.da_thay.get(&b)) {
            (Some(x), Some(y)) => (x.clone(), y.clone()),
            _ => return 0,
        };
        let mut trao_doi = 0;
        for m in &ca { if self.da_thay.get_mut(&b).unwrap().insert(*m) { trao_doi += 1; } }
        for m in &cb { if self.da_thay.get_mut(&a).unwrap().insert(*m) { trao_doi += 1; } }
        trao_doi
    }

    /// Chạy anti-entropy tới khi mọi nút hội tụ, hoặc hết `toi_da` vòng.
    pub fn hoi_tu(&mut self, toi_da: usize) -> Option<usize> {
        let cac_nut: Vec<MaNut> = self.da_thay.keys().copied().collect();
        let n = cac_nut.len();
        if n == 0 { return Some(0); }
        for vong in 1..=toi_da {
            ...
            for i in 0..n {
                let (a, b) = (cac_nut[i], cac_nut[(i + vong) % n]);
                self.chong_entropy(a, b);
            }
            let dich = self.da_thay[&cac_nut[0]].len();
            if self.da_thay.values().all(|x| x.len() == dich) { return Some(vong); }
        }
        None
    }
}
...
pub struct BoBatDoiMat {
    ...
    da_thay: HashMap<(usize, u64), u64>,
    pub bang_chung: Vec<BangChungDoiMat>,
}

impl BoBatDoiMat {
    pub fn quan_sat(&mut self, nut: usize, vong: u64, gia_tri: u64) -> bool {
        match self.da_thay.get(&(nut, vong)) {
            Some(&cu) if cu != gia_tri => {
                self.bang_chung.push(BangChungDoiMat {
                    nut, vong, gia_tri_a: cu, gia_tri_b: gia_tri,
                });
                true // phát hiện đôi mặt
            }
            Some(_) => false,          // lặp lại đúng giá trị cũ — hợp lệ
            None => { self.da_thay.insert((nut, vong), gia_tri); false }
        }
    }
